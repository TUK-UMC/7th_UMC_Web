...
import { creditType } from "src/types/movieTypes";
...
export const MovieDetail = () => {
  ...
  const {
    ...
    queryFn: () => movieId && getMovieDetail(movieId),
  });
  ...
  const {
    ...
    queryFn: () => movieId && getMovieCredits(movieId),
  });
  ...
  return (
    <Container>
      ...
      <CreditsWrapper>
        <h3>감독/출연</h3>
        <CreditsScroll>
          {credits?.cast?.map((cast: creditType) => (
            <CircleProfile
              name={cast.name}
              key={cast.id}
